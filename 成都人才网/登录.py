import base64
import binascii
import requests
import ddddocr
import execjs
from PIL import Image
from io import BytesIO
from gmssl import sm2

# ------------------配置区------------------
USERNAME = "18063514542"
PASSWORD = "111111"

PUBLIC_KEY = "04CF18973EA8F51C60B5595D728BE3342B802E9275B34BCFC2B69E39099F5E624049C492FFF7FB1DCD9C999F29D26AC7121F438DE0F959AF49F8A92C9F3C11D4BB"

LOGIN_URL = "https://member.rc114.com/User/LoginForm"
VALIDCODE_URL = "https://member.rc114.com/Home/GetValidcode"

HEADERS = {
    "Accept": "application/json, text/javascript, */*; q=0.01",
    "Accept-Language": "zh-CN,zh;q=0.9",
    "Cache-Control": "no-cache",
    "Connection": "keep-alive",
    "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
    "Origin": "https://member.rc114.com",
    "Pragma": "no-cache",
    "Referer": "https://member.rc114.com/",
    "Sec-Fetch-Dest": "empty",
    "Sec-Fetch-Mode": "cors",
    "Sec-Fetch-Site": "same-origin",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36",
    "X-Requested-With": "XMLHttpRequest",
    "sec-ch-ua": "\"Chromium\";v=\"130\", \"Google Chrome\";v=\"130\", \"Not?A_Brand\";v=\"99\"",
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": "\"Windows\""
}

COOKIES = {
    "Hm_lvt_93f4034e873979432681a4780bea5b22": "1751354774,1751354807",
    "rc_sys": "rc114",
    "Hm_lvt_57c48c2407b9363122624c39f3e251df": "1754572551",
    "HMACCOUNT": "29C8E5B0264CE645",
    "Hm_lpvt_57c48c2407b9363122624c39f3e251df": "1754572567",
    "ASP.NET_SessionId": "fgpcxrveiele3gw5t030qnum",
    "returnUrl": "http%3A%2F%2Fmember.rc114.com%3A804%2FUser%2FIndex"
}
# ------------------------------------------


# ========== STEP 1: 获取验证码图片 ==========
resp = requests.get(VALIDCODE_URL, headers=HEADERS, cookies=COOKIES)
base64_url = resp.json()["ValidImage"]
image_data = base64.b64decode(base64_url)

# PIL 读入并转字节流
img = Image.open(BytesIO(image_data))
img_bytes_io = BytesIO()
img.save(img_bytes_io, format='PNG')
img_bytes = img_bytes_io.getvalue()

# ========== STEP 2: OCR识别点击坐标 ==========
ocr = ddddocr.DdddOcr(det=True, ocr=False)
boxes = ocr.detection(img_bytes)

def center(box):
    x1, y1, x2, y2 = box
    return [int((x1 + x2) / 2), int((y1 + y2) / 2)]

coords = [center(box) for box in boxes]

# ========== STEP 3: JS处理 validateCode ==========
with open("captcha.js", "r", encoding="utf-8") as f:
    js_code = f.read()

ctx = execjs.compile(js_code)
validate_code = ctx.call("dealMapArr", coords)

# ========== STEP 4: 密码 SM2 加密 ==========
def sm2_encrypt(data: str, public_key: str, cipher_mode: int = 0) -> str:
    if public_key.startswith('04'):
        public_key = public_key[2:]
    if len(public_key) != 128:
        raise ValueError("公钥必须是128个十六进制字符")
    full_public_key = '04' + public_key
    sm2_crypt = sm2.CryptSM2(public_key=full_public_key, private_key='')
    encrypted_bytes = sm2_crypt.encrypt(data.encode('utf-8'))
    encrypted_hex = binascii.b2a_hex(encrypted_bytes).decode().upper()
    if cipher_mode == 0:
        c1 = encrypted_hex[:130]
        c3 = encrypted_hex[-64:]
        c2 = encrypted_hex[130:-64]
        encrypted_hex = c1 + c3 + c2
    return '04' + encrypted_hex

pwd_encrypt = sm2_encrypt(PASSWORD, PUBLIC_KEY, cipher_mode=0)

# ========== STEP 5: 登录提交 ==========
data = {
    "userName": USERNAME,
    "pwdEncrypt": pwd_encrypt,
    "screenDpi": "1536 × 864",
    "returnUrl": "http://member.rc114.com:804/User/Index",
    "validateCode": validate_code
}

login_resp = requests.post(LOGIN_URL, headers=HEADERS, cookies=COOKIES, data=data)
print("登录响应：", login_resp.text)
print("状态码：", login_resp.status_code)
